...
type ActionType =
  ...
  | 'setNowHp'
  | 'addKammusuSetting'
  | 'deleteKammusuSetting'
  | 'setFinalAttackList'
  | 'setCriticalPer'
  | 'setFinalAttackInputData';
...
interface ISettingContext {
  ...
  chartData: ChartData<'scatter', ScatterPoint[]> | null;
  chartOption: ChartOptions<'scatter'> | null;
...
const createChartDataSets = (
  ...
  return {
    backgroundColor: toRgba(CHART_COLORS[colorIndex], 0.2),
 ...
 */
const findCropRange = (chart: ChartDataset<'scatter', ScatterPoint[]>): { min: number; max: number } => {
  let minValue = MIN_FINAL_ATTACK;
  let maxValue = MAX_FINAL_ATTACK;
  const chartData = (chart.data ?? []) as ScatterPoint[];
...
export const useSettingState = (): ISettingContext => {
  ...
  const [chartData, setChartData] = useState<ChartData<'scatter', ScatterPoint[]> | null>(null);
  const [chartOption, setChartOption] = useState<ChartOptions<'scatter'> | null>(null);
  const [finalAttackInputData, setFinalAttackInputData] = useState('');

  useEffect(() => {
    const chartDataSetList: ChartDataset<'scatter', ScatterPoint[]>[] = [];

    const primaryChartDataSets = createChartDataSets(maxHp, armor, nowHp, graphName);
    ...
    for (let colorIndex = 1; colorIndex < CHART_COLORS_SIZE; ++colorIndex) {
      ...
      const secondaryChartDataSets = createChartDataSets(
        kammusuSetting.maxHp,
        kammusuSetting.armor,
        kammusuSetting.nowHp,
        kammusuSetting.graphName,
        colorIndex
      );
    ...
    setChartData({ datasets: chartDataSetList });
  ...
  useEffect(() => {
    setChartOption({
      ...
      scales: {
        x: {
          title: { display: true, text: '最終攻撃力' },
          min: minValue,
          max: maxValue
        },
        y: {
          title: { display: true, text: '大破率(％)' }
        }
      }
    });
  }, [minValue, maxValue]);
  ...
  const dispatch = (action: Action) => {
    switch (action.type) {
      ...
      case 'deleteKammusuSetting': {
        const index = parseInt(action.message as string, 10);
