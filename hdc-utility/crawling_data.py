...
# 深海棲艦の装備一覧のURL
ENEMY_DATA_URL = 'http://kancolle.wikia.com/wiki/List_of_equipment_used_by_the_enemy'
...
def read_parameters(td_tag: lxml.html.HtmlElement) -> Dict[str, int]:
    """装備のパラメーターを読み取る
...
def get_enemy_weapon_dict() -> Dict[int, Weapon]:
    ...
    # URLを読み取り、HTMLをパースする
    response = requests.get(ENEMY_DATA_URL)
    ...
    for tr_tag in dom.cssselect('table.wikitable tr'):
        ...
        # 装備種を読み取る
        raw_weapon_type = td_tag_list[3].text.replace('\n', '')
        weapon_type = WEAPON_TYPE_DICT.get(raw_weapon_type, WeaponType.NONE)

        # 他のパラメーターを読み取る
        parameters = read_parameters(td_tag_list[4])
